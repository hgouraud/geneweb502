...
value print_short conf base list len =
  ...
  in
  let _ = print_endline "build_ini" in
  let ini_list = build_ini list (String.length ini) in
  let _ = print_endline "affichage...." in
  do {
    let title _ = print_title conf base (Mutil.tr '_' ' ' ini) len in
    Hutil.header conf title;
    print_link_to_welcome conf True;
...
value print_mod conf base = 
  ...
  let _ = print_endline "get_all_data" in
  ...
  let _ = print_endline "rev_map" in
  ...
  let _ = print_endline "combine" in
  ...
  let reduce l =
    List.fold_left
      ...
      [] l
  in
  let list = if ini <> "" then   let _ = print_endline "reduce" in reduce list else list in
  let len = List.length list in
  if len > max_results then   let _ = print_endline "print_short" in print_short conf base list len
  else   let _ = print_endline "print_long" in print_long conf base list len
;
